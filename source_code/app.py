...
def initialize():
    ...
    # TODO: The automatic discovery is too unreliable, so IPs are hardcoded for now
    ...
    print('IPs sorted by cam position: ', [(conn.ip, conn.cam_position) for conn in connections])
...
def show_add_contact_info(gif_file_name):
    return render_template('add_contact_info.html', gif_file_name=gif_file_name)
...
def snap_a_gif():
    ...
    print("URL FOR", url_for('show_add_contact_info', gif_file_name=gif_file_name))
    return redirect(url_for('show_add_contact_info', gif_file_name=gif_file_name))
